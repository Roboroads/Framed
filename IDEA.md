...
This is a game played in real life, assisted by your phone's GPS. Players gather at a starting point and start a game, after which they have to disperse. After the dispersion timer, the game starts for real. Everyone gets a target they have to find and "frame" by taking a picture of them. The picture is sent to the other players to judge whether it really shows the target. On a majority "yes" vote, the target is out of the game and the framer inherits their target's target. The game ends when one player remains; who wins depends on the game mode.
...
The host picks one of two modes in the lobby. Gameplay is identical in both — same circle, frames, votes, punishments, and being framed is permanent. Only the winner differs:

- **Most frames wins** (default). When the game ends, the player with the most confirmed frames wins — a framed player can win. Ties break on survival time, so the last player standing beats anyone tied with them.
- **Last man standing.** The last unframed player wins.

Either way the game ends when one player remains (or when the last players go MIA simultaneously).

## Core loop: framing & judging

1. Framer opens the in-app camera from the ingame screen and photographs their target. **In-app camera only** — no gallery uploads.
2. The photo goes to all other players (unframed **and** framed) except the framer and the target. Judges see the frame photo **side by side with the target's reference selfie**: "Is this (player name)?" with ✗ / ✓ buttons.
...
4. **Vote passes:** the target is framed, the framer inherits the target's target and gets a fresh compass update for them.
5. **Vote fails:** the framer gets a frame cooldown (`frame_cooldown_minutes`) before they can submit another photo. Nothing else happens — the target is not informed.

While a vote about them is pending, the target is **not notified** and keeps playing normally — but any frame *they* submit during that window is held until their own verdict resolves (the framed can't frame). Only one pending frame per framer at a time.

## The compass

Once every `compass_update_interval_minutes` a **global pulse** fires: every unframed player simultaneously receives a compass update — an arrow and distance to their target's location *at pulse time* (a snapshot; it does not track the target live). The arrow rotates live with the device compass. The update disappears after `compass_view_seconds` seconds.
...
- Verdict on your submitted frame (frame confirmed / rejected + cooldown).
- Rule-break warning (geofence, stale location) and punishment escalation.
- You got framed / new target inherited / game finished.
...
- **Ingame.** This screen has multiple states:
  ...
  - **Warning modal.** Shown while you're breaking a rule (e.g. outside the geofence) until you stop or you're framed.
...
- **Framed screen.** How you got framed, how long you survived, the photo that framed you, **who your framer was**, and who else is currently out (a one-time snapshot on load, not live-updated). Includes a chat with the other framed players so they can set up a meeting spot. Framed players still receive judging modals. A confirmed "Leave game" button is here too, with a note that leaving might end the game early for everyone (see Game rules).
- **Game finish.** Winner (per the game mode, with the mode named on screen) plus stats (most frames, most stand-still player, most moved player, combined movement, etc.) and the full frame chain. The host can choose "replay with same players" (same lobby, same selfies, reshuffled targets); players can only "Leave game". The same chat from the framed screen carries over here too, open to everyone now regardless of whether they got framed, so the whole group can coordinate a meetup once the game's actually over.
...
- `hard_punishment_minutes` — how long you can break a rule before you're framed automatically. It counts as being framed, but the framed screen shows "MIA (broke a game rule for too long)". Default: 5.
...
- **Stay inside the play area.**
  ...
  - Soft punishment: your framer receives your exact location on the map.
  - Hard punishment: out as MIA.
...
- **Your phone must send location updates to the server.**
  ...
  - While stale: the warning modal shows, and you receive no compass update at a pulse. Your framer's compass still points at your last known location.
  - Stale for `hard_punishment_minutes` straight → out as MIA (this covers dead phones and force-closed apps). Any successful update resets the timer.
  - Reopening the app after a crash or force-close resumes exactly where you left off (#54) — the app itself doesn't frame you for going quiet. But nothing pauses the staleness clock while you're gone, so silence for `hard_punishment_minutes` still takes you out as MIA whether that silence is a crash, a dead phone, or someone deliberately sitting out. Reconnecting only saves you if you're back before that timer runs out.
- **Anyone can leave, with a confirmation dialog first.** The lobby, every ingame screen (dispersing, actively hunting, framed), and the finish screen all offer a "Leave" button, none of them one tap away. What it costs depends on when:
  - Leaving the lobby or the finish screen just gives up your seat — no game consequences.
  - Leaving while framed drops you out of the judge pool for every future vote.
  - Leaving while still in the game (dispersing or actively hunting) takes you out outright, cause "left" — not framed by a player, not MIA. Whoever's hunting you inherits your target exactly like any other elimination, and any frame you had in flight as framer is voided: the same relink `kill_player` already runs for every elimination, just triggered on demand instead of by a timer.
  - Either way, if leaving drops the game's roster (everyone who hasn't left) below 3 players, there's no way to guarantee a judge for whoever's left, so the game ends immediately: whoever currently has the most confirmed frames wins, the same "tolerates a framed winner" rule as most-frames mode.
...
- **Votes are final.**
  - A majority "yes" frames the target even if the judges were wrong. There is no appeal; the vote is the truth.
- **Dispersal phase.**
...
- **Final duel (2 left):** both players target each other; the framed are the jury. Still holds as long as the roster (everyone who hasn't left) stays at 3 or more — below that, the low-player-count rule above ends the game first.
- **Target framed by someone else while a frame on them is pending:** impossible — each player has exactly one framer in the circle.
- **Framer goes MIA while their frame is pending:** the pending vote is cancelled.
- **Simultaneous pending frames** (A framed B while B's frame of C is pending): B's held frame resolves after B's own verdict — if B is framed, B's frame is void.
...
- **Cleanup.** After a game, all information (pictures, game data) is deleted. "Replay with same players" carries the lobby (names, selfies) into the next game — deletion happens when the lobby disbands. Games running longer than 24 hours are auto-cleaned the same way; we keep an anonymous counter of how often this triggers. A lobby that never starts expires much sooner: an hour of nobody in it sending a heartbeat wipes it outright, and an individual player who's gone quiet for 15 minutes is dropped from an otherwise-active lobby the same way leaving it themselves would. Nothing else is kept except stats that cannot be traced back to players or locations, for "fun stats" purposes (how much the game gets played, how long, etc.).
...
- **Encrypted client-side before upload:** display names, reference selfies, frame photos, and framed-chat messages. The server stores and relays opaque blobs.
...
- **What we process:** display name, reference selfie, live GPS location, frame photos, push token, framed-chat messages. All of it is personal data (faces + location are sensitive), all of it is ephemeral: deleted at game end, by the 24-hour auto-cleanup, or sooner still for a lobby that never starts (an hour of inactivity, or 15 minutes for a single quiet player) — whichever comes first. Names, selfies, photos, and chat are end-to-end encrypted — the server operator cannot read them even while they exist. Locations are readable server-side (the server enforces the game with them) but only last-known is stored.
- **Lawful basis:** consent. The pre-join screen states plainly what is collected and how long it lives, in full view of the join button — and nothing leaves the phone until that button is pressed. Joining a game is the consent action. Leaving the lobby before game start deletes your data immediately; quitting mid-game makes you MIA and everything is wiped at game end (24h max) — the right to erasure, automated.
...
- **Powerups.** Shown on the map; claim one by standing at that spot (landmark?) and use it from the ingame screen. Powerups should be individually toggleable per-game. Candidate set: 
  ...
  - Radar (you get a compass hint for where your framer is)
  - Hourglass (shows exact location of target 15 minutes ago, target gets notified)
  - Spyglass (Tells you who your framer is, framer gets notified)
...
- **Spectator map for the framed.** Framed players see a live map with everyone's position plus the frame feed, turning the framed screen + chat into a spectator lounge.
- **Kill feed announcements.** Anonymous broadcasts to all players: "A player has been framed. 6 remain."
- **End-of-game replay map.** An animated timeline of all movement and frames on the finish screen, shown once before the privacy wipe deletes the data.
