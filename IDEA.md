...
- **Death screen.** How you died, how long you survived, the photo that framed you, **who your assassin was**, and who else is currently out (a one-time snapshot on load, not live-updated). Includes a chat with the other dead players so they can set up a meeting spot. Dead players still receive judging modals. A confirmed "Leave game" button is here too, with a note that leaving might end the game early for everyone (see Game rules).
- **Game finish.** Winner (per the game mode, with the mode named on screen) plus stats (most kills, most stand-still player, most moved player, combined movement, etc.) and the full kill chain. The host can choose "replay with same players" (same lobby, same selfies, reshuffled targets); players can only "Leave game". The same chat from the death screen carries over here too, open to everyone now regardless of whether they died, so the whole group can coordinate a meetup once the game's actually over.
...
- **Your phone must send location updates to the server.**
  ...
  - Reopening the app after a crash or force-close resumes exactly where you left off (#54) — the app itself doesn't kill you for going quiet. But nothing pauses the staleness clock while you're gone, so silence for `hard_punishment_minutes` still kills you as MIA whether that silence is a crash, a dead phone, or someone deliberately sitting out. Reconnecting only saves you if you're back before that timer runs out.
- **Anyone can leave, with a confirmation dialog first.** The lobby, every ingame screen (dispersing, actively hunting, dead), and the finish screen all offer a "Leave" button, none of them one tap away. What it costs depends on when:
  - Leaving the lobby or the finish screen just gives up your seat — no game consequences.
  - Leaving while dead drops you out of the judge pool for every future vote.
  - Leaving while alive (dispersing or actively hunting) kills you outright, cause "left" — not framed, not MIA. Whoever's hunting you inherits your target exactly like any other death, and any frame you had in flight as assassin is voided: the same relink `kill_player` already runs for every death, just triggered on demand instead of by a timer.
  - Either way, if leaving drops the game's roster (everyone who hasn't left) below 3 players, there's no way to guarantee a judge for whoever's left, so the game ends immediately: whoever currently has the most confirmed frames wins, the same "tolerates a dead winner" rule as most-frames mode.
